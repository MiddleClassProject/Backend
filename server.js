...
// cus 테이블이 없으면 생성하는 함수
function createCusTableIfNotExists() {
    pool.getConnection((err, conn) => {
        ...
        const createCusTableQuery = `
        CREATE TABLE IF NOT EXISTS cus (
            cus_id VARCHAR(100) PRIMARY KEY COMMENT '사용자 로그인 아이디',
            cus_name VARCHAR(100) NOT NULL COMMENT '사용자 이름',
            cus_age SMALLINT UNSIGNED NOT NULL COMMENT '사용자 나이',
            cus_password VARCHAR(100) NOT NULL COMMENT '로그인 암호',
            CONSTRAINT FK_cus_user FOREIGN KEY (cus_id) REFERENCES users(user_id) ON DELETE CASCADE
        ) 
    `;


        conn.query(createCusTableQuery, (err, result) => {
            conn.release();
            if (err) {
                console.log('cus 테이블 생성 실패:', err);
            } 
        });
    });
}

// pro 테이블이 없으면 생성하는 함수
function createProTableIfNotExists() {
    pool.getConnection((err, conn) => {
        if (err) {
            console.log('MySQL connection error');
            return;
        ...
        const createProTableQuery = `
        CREATE TABLE IF NOT EXISTS pro (
            pro_id VARCHAR(100) PRIMARY KEY COMMENT '교수 로그인 아이디',
            pro_name VARCHAR(100) NOT NULL COMMENT '교수 이름',
            pro_age SMALLINT UNSIGNED NOT NULL COMMENT '교수 나이',
            pro_password VARCHAR(100) NOT NULL COMMENT '로그인 암호',
            pro_path VARCHAR(255) NOT NULL COMMENT '증명서 파일 경로',
            pro_check TINYINT(1) DEFAULT '0' COMMENT '증명서 확인 여부',
            CONSTRAINT FK_pro_user FOREIGN KEY (pro_id) REFERENCES users(user_id) ON DELETE CASCADE  -- 외래 키 설정
        ) 
    `;

        conn.query(createProTableQuery, (err, result) => {
            conn.release();
            if (err) {
                console.log('pro 테이블 생성 실패:', err);
            } 
        });
    });
}

// cus, pro ID 저장 users 테이블
    ...
    pool.getConnection((err, conn) => {
        ...
        const createUsersTableQuery = `
            CREATE TABLE IF NOT EXISTS users (
                user_id VARCHAR(100) PRIMARY KEY COMMENT '사용자 또는 교수 ID',
                user_type VARCHAR(3) NOT NULL COMMENT '사용자 유형 (cus=고객, pro=교수)'
            ) 
        `;

        conn.query(createUsersTableQuery, (err, result) => {
            conn.release();
            if (err) {
                console.log('users 테이블 생성 실패:', err);
            }
        });
    });
}
...
createCusTableIfNotExists();
createProTableIfNotExists();
